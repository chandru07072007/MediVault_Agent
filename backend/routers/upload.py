...
def _get_user_bucket_context(
    ...
    if settings.USE_MOCK_S3:
        # In mock mode, preserve the bucket selected in UI for history/accounting.
        return None, ((preferred_bucket or "").strip() or settings.S3_BUCKET_NAME)

    strict_preferred_lookup = strict_preferred
    if preferred_bucket and preferred_bucket == settings.S3_BUCKET_NAME:
        # If selected bucket is the system default, allow fallback to default credentials
        # when there is no saved per-user credential record.
        strict_preferred_lookup = False
    ...
    if not bucket_record:
        if preferred_bucket and strict_preferred_lookup:
