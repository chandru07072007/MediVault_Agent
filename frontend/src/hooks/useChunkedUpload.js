...
function normalizeUploadError(err) {
  ...
  if (err?.code === "S3_ETAG_MISSING") {
    return {
      kind: "s3_cors",
      status,
      message:
        "Upload reached S3 but ETag header was not readable. Configure bucket CORS to expose ETag for http://localhost:5173.",
      occurredAt: Date.now(),
    };
  }
  ...
  if (err?.code === "S3_PUT_FAILED") {
    return {
      ...
      status,
      message:
        "Browser could not send chunk to S3. Check S3 bucket CORS (PUT/POST/HEAD/GET and allowed origin http://localhost:5173).",
      occurredAt: Date.now(),
    };
  }
  ...
  if (err?.code === "AUTH_ERROR" || status === 401 || status === 403) {
    return {
      kind: "auth",
      status,
      message: "Authentication is required to continue this upload. Sign in again and click Resume.",
      occurredAt: Date.now(),
    };
  }
...
export function useChunkedUpload() {
  ...
  const uploadChunk = async (chunk, fileId, fileKey, uploadId, runId, retryCount = 0) => {
    try {
      ...
      if (!etag) {
        const etagError = new Error("No ETag returned from S3");
        etagError.code = "S3_ETAG_MISSING";
        etagError.status = response.status;
        throw etagError;
      }
      ...
      if (err instanceof TypeError && !extractStatus(err)) {
        const s3PutError = new Error("Network or CORS blocked S3 upload request");
        s3PutError.code = "S3_PUT_FAILED";
        s3PutError.cause = err;
        throw s3PutError;
      }
      ...
      if (status === 401 || status === 403) {
        try {
          ...
          return uploadChunk(chunk, fileId, fileKey, uploadId, runId, retryCount);
        } catch {
          // If refresh fails, propagate auth error to pause flow for re-login.
        }
